/*
  QRCode for JavaScript by Kazuhiko Arase, MIT License.
  Bundled locally for Moto Talk QR Offline. The word QR Code is a registered trademark of DENSO WAVE INCORPORATED.
*/
(function(global){
  var modules = {};
  function require(name){ if(!modules[name]) throw new Error('Module not found: ' + name); return modules[name]; }
  function define(name, factory){ var module = { exports: {} }; factory(module, module.exports, require); modules[name] = module.exports; }
...
module.exports = QRCode;
  ...
  function draw(canvas, text) {
    ...
    var maxPixels = Math.min(900, Math.max(300, Math.floor((window.innerWidth || 360) - 48)));
    var scale = Math.max(2, Math.floor(maxPixels / size));
    canvas.width = size * scale;
    canvas.height = size * scale;
    canvas.style.width = Math.min(canvas.width, maxPixels) + 'px';
    ...
    return { modules: count, pixels: canvas.width };
  }
  global.MotoQR = { draw: draw };
})(window);
