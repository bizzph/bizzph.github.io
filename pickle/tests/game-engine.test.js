...
test('normalization constrains hostile imported fields to the game schema', () => {
  const normalized = Engine.normalizeGame({
    id: '<img src=x onerror=alert(1)>',
    format: '<svg onload=alert(1)>',
    target: '<script>alert(1)</script>',
    status: 'owned',
    unknown: { executable: true },
    teams: [
      { name: '<img src=x>', players: ['<b>Ava</b>', 'Ben'], score: '<img src=x>' },
      { name: 'B', players: ['Cora', 'Drew'], score: 1e12 }
    ],
    rallies: new Array(Engine.MAX_RALLIES + 20).fill({ winnerTeam: 0, before: {} })
  });
  assert.equal(normalized.format, 'doubles');
  assert.equal(normalized.target, 11);
  assert.equal(normalized.status, 'active');
  assert.equal(normalized.teams[0].score, 0);
  assert.equal(normalized.teams[1].score, Engine.MAX_SCORE);
  assert.equal(normalized.rallies.length, Engine.MAX_RALLIES);
  assert.equal(Object.hasOwn(normalized, 'unknown'), false);
  assert.match(normalized.id, /^[A-Za-z0-9_-]+$/);
});

test('public live state excludes rally history and arbitrary properties', () => {
  let value = game();
  value = Engine.recordRally(value, 0, 2000);
  value.privateNotes = 'do not broadcast';
  const publicValue = Engine.publicGameState(value);
  assert.deepEqual(publicValue.rallies, []);
  assert.equal(Object.hasOwn(publicValue, 'privateNotes'), false);
  assert.equal(Object.hasOwn(publicValue.teams[0], 'playerIds'), false);
  assert.equal(publicValue.teams[0].score, 1);
});

test('game imports reject unbounded history', () => {
  assert.throws(
    () => Engine.validateImport({ games: new Array(Engine.MAX_IMPORT_GAMES + 1).fill({ teams: [{}, {}] }) }),
    /at most/
  );
});
